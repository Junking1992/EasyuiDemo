...
public class ExcelControllerNoWeb{
	/**
	 * 将待转换Excel此路径
	 */
...
//			}
	...
	public static void main(String[] args) {
		Runnable parse = new Runnable() {
			public void run() {
				try {
					ExcelControllerNoWeb web = new ExcelControllerNoWeb();
					List<Map<String, String>> data;
					ProgressUtil main = new OpeningByXlsNew();
					//从第三行开始读取数据(第一行为0)
					web.startRowNum = 2;
					File file = new File("E:/UploadExcel/excle");
					File[] files = file.listFiles();
					for(File f : files){
						System.out.println("正在导入:" + f.getName());
						main.fileName = f.getName();
						data = web.parseExcel("E:/UploadExcel/excle/" + f.getName());
						main.initIquantityDate(data);
						System.out.println(f.getName() + "导入完成!");
					}
				} catch (Exception e) {
					//把特殊异常放在前面
					e.printStackTrace(); 
